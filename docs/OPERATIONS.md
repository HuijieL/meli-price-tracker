...
**状态**：每日定时跑（巴西 7:30 抓数据 + 8:00 前发邮件），无需人工介入
...
啥都不做。Actions 一条龙：fetch → commit → generate-report → email。两个 cron：

- **主**：`30 10 * * *` = UTC 10:30 = 巴西 7:30（每天按时跑，预留 30min 让邮件 8:00 前抵达）
- **备**：`0 13 * * *` = UTC 13:00 = 巴西 10:00（主若被 GH 丢弃则兜底）

备份 cron 会先检查 `data/daily/{今日巴西日期}.json` 是否已存在：存在则**所有 step 全 skip**（包括邮件步骤）空跑退出；不存在才完整抓+发一遍。幂等，零成本，不会重复发邮件。

> **历史注**：
> - 2026-04-19 前用的是 `0 0 * * *`（UTC 00:00 / 巴西 21:00）。该时段是 GitHub Actions 调度高峰，scheduled workflows 常被静默丢弃，4-17/4-18 都碰过。改到避开整点。
> - 2026-04-19~2026-05-05 用 `1 23 * * *`（UTC 23:01 / 巴西 20:01），云端只抓数据，邮件靠本地 `/meli`。
> - 2026-05-06 起改为 `30 10 * * *`（UTC 10:30 / 巴西 7:30），fetch + email 一条龙，本地 `/meli` 改为按需备用。
