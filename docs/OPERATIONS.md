...
啥都不做。Actions 每天 UTC 23:01（巴西 20:01）自动跑，数据 commit 到仓库。邮件由 `/meli` skill 本地触发时发送。

> **历史注**：2026-04-19 前用的是 `0 0 * * *`（UTC 00:00 / 巴西 21:00）。该时段是 GitHub Actions 调度高峰，scheduled workflows 容易被静默丢弃，4-17/4-18 都出现过该问题。改到 `1 23 * * *`（UTC 23:01 / 巴西 20:01）避开整点。
